...
def scan_stock(symbol, vol_mult, tp_pct, sl_pct,
	...
	try:
		...
		avg_vol20 = volume.rolling(21).mean()
		...
		for i in range(len(df) - 2, 20, -1):
			...
			# === Breakout search AFTER HV candle ===

			    # If structure breaks before entry → invalidate setup
				# Monitor indefinitely after HV candle
			for j in range(i + 1, len(df)):

			    # Entry condition: close above EMA10
			    if close.iloc[j] > ema10.iloc[j]:

			        entry_price = close.iloc[j]
			        entry_date = df.index[j]

			        # NEW stop logic (5% from entry)
